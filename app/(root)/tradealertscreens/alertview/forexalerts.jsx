...
const ForexAlerts = () => {
    ...
    const forexRates = rates?.forex || [];
    return (
        <View style={styles.container}>
            {forexRates.length === 0 ? (
                <Text style={styles.noData}>No forex alerts available</Text>
            ) : (
                <FlatList
                    data={forexRates}
                    renderItem={({ item }) => (
                        <TouchableOpacity
                            style={styles.item}
                            onPress={() => navigation.push('fxdetails', { currencyPair: item.name, assetType: 'forex' })}
                        >
