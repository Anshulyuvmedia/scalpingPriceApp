// app/(root)/(tabs)/chatbot/tabview/StocksTab.jsx
...
const GradientCard = ({ children, style, gradientColor }) => (
    <LinearGradient
        colors={['#000', gradientColor || '#AEAED4']}
        start={{ x: 1, y: 0.6 }}
        end={{ x: 0, y: 0.5 }}
...
const StocksTab = ({ signals, loading, error, onRefresh }) => {
    ...
    if (signals.length === 0) {
        return (
            <View className="flex-1 bg-black justify-center items-center">
                <Text className="text-white font-questrial text-lg">No Stocks signals available.</Text>
            </View>
        );
    }
